...
namespace sol
{
    ...
    void read()
    {
        ...
        {
            cin >> f[i];
            wstp[f[i]].push_back(i);
        }
    }

    inline int real_ind(int x)
    {
        return x + ind::query(x);
    }
    
    inline void make_real(int x, int ri)
    {
        while (len(wstp[x]))
        {
            if (real_ind(wstp[x].front()) >= ri)
                break;
            wstp[x].pop_front();
        }
    }
    ...
    bool solve()
    {
        for (int i = 1; i <= n;)
        {
            ...
            int en = wstp[x].front(), ren = real_ind(en);
            ...
            for (auto j : g[x])
            {
                ...
                auto ind = real_ind(wstp[j].front());

                if (inRange(ind, ri, ren))
                    return false;
            }
